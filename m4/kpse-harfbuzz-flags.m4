...
[_KPSE_LIB_FLAGS([harfbuzz], [harfbuzz], [lt],
...
[m4_ifval([$1],
          [AC_ARG_WITH([system-harfbuzz],
                       AS_HELP_STRING([--with-system-harfbuzz],
                                      [use installed harfbuzz headers and library (requires pkg-config)]))])[]dnl
...
if $PKG_CONFIG harfbuzz --atleast-version=0.9.7; then
  ...
  AC_MSG_ERROR([did not find harfbuzz-0.9.7 or better])
fi
]) # KPSE_HARFBUZZ_SYSTEM_FLAGS
