/* afm2pl
 * Copyright (C) 2002, 2004 Siep Kroonenberg
 * ntg-afm2pl@ntg.nl
 * based on afm2tfm.
 */
...
/* SK:
 * string.h moved outside ifthenelse block
 */
#ifdef KPATHSEA
#include "c-auto.h"
 ...
 */
typedef enum {
  kpse_afm_format, kpse_tex_ps_header_format, kpse_lig_format
...
P1H (void)
{
  ...
  if (encfilename) {
    encfilename =
      openin (encfilename, kpse_tex_ps_header_format, ".enc");
    param = 0;
    if (infile == 0)
...
#endif
  fputs ("afm2pl(k) 0.7.02\n", f);
#ifdef KPATHSEA
  fprintf (f, "%s\n", kpathsea_version_string);
#endif
  fputs ("Copyright (C) 2002, 2004 Siep Kroonenberg.\n\
...
#define CHECKARG3 if (argc < 3) { usage(stderr); exit(1); }
...
P2C (int, argc, char **, argv)
{
  ...
  if (!strcmp (argv[0], "--version") || (!strcmp (argv[0], "-v"))) {
    version (stdout);
    exit (0);
  }
