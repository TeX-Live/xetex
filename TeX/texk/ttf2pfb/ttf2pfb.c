...
#include "big52uni.h"         /* add support Big5 Chinese Unicode ttf fonts */
...
#endif
  mesg("-a\t\tPrint the index of glyphs.\n");
  mesg("-gbk,-GBK\tGenerate GBK encoded fonts from Unicode TTF files.\n");
  mesg("-big5,-BIG5\tGenerate BIG5 encoded fonts from Unicode TTF files.\n");
        ...
        int UID)
{
  ...
  if(CHINESE && !compact)
  	fprintf(out, "/Notice (Plane %02x) readonly def\n", plane);
  else
...
tmp_out(FILE* tmpout)
{
  ...
  Fputps("%%% Generated by: ttf2pfb $Revision: 1.14 $");
...
void writeEncFile(char *psfile)
{
    int i;
    char enc_name[1024], *p;
    if (psfile != NULL)
        strcpy(enc_name, psfile);
    else
        strcpy(enc_name, "stdout");
    if ((p = strrchr(enc_name, '.')) == NULL)
        p = strchr(enc_name, 0);
    *p = 0;
    strcpy(p, ".enc");
     ...
     char *argv[])
{
  ...
  if (make_enc && result == 0)
      writeEncFile(outfile);
