...
const char *const xdvirc_header = ""
	...
	"!!! the entries in your ~/.Xdefaults file (but not the command-line\n"
	"!!! arguments passed to xdvi). Remove this file if you want\n"
	"!!! to get rid of all these customizations.\n"
	"!!!\n";
...
save_user_preferences(Boolean full_save)
{
	...
	if ((from_fp = try_fopen(tmpname, "r")) == NULL) {
	    XDVI_ERROR((stderr, "opening %s for reading failed", tmpname));
	    return True;
	}
	
	if ((to_fp = try_fopen(xdvirc_name, "w+")) == NULL) {
	    XDVI_ERROR((stderr, "opening %s for writing failed", xdvirc_name));
	    return True;
	}
	...
	if (fputs(xdvirc_signature_line, to_fp) == EOF
	    ...
	    XDVI_ERROR((stderr, "error writing to %s: %s", xdvirc_name, strerror(errno)));
	}
