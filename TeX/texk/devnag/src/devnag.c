/*  devnag.c v2.01: November 29, 2001
 *
 ...
 *
 ...
 Date     : 09 May 1991
 *
 The maintainer of this program is now John Smith <jds10@cam.ac.uk>.
 *
...
short joincode;
unsigned char hindi_mode, dollar_mode, d_found;
...
short nolig[] = {
   2,3,6,7,16,25,27,34,35,39,40,46,47,49,
 ...
 */
short ligidxs[] = {
   0,1,2,3,4,6,7,9,11,12,13,14,118,16,17,18,
...
typcom TabCom[] = {
     ...
     {1,"enlargethispage"},
     {1,"enlargethispage*"},
     {1,"thispagestyle"}, /*ajouté le 19-12-01*/
     {1,"include"},
     {1,"input"},
     {1,"parbox"},
     {1,"stepcounter"},
     ...
     {1,"bibitem"},
     {1,"cline"},
     {1,"vadjust"}, /*ajouté le 05-01-02*/
     {1,"retrait"}, /*ajouté le 26-01-02*/
     {2,"rm"},
     {2,"kern"},
     {2,"hskip"},
     {2,"vskip"},
     ...
     {3,"rule"},
     {3,"addcontentsline"},
     {3,"notel"},/*essai de note longue 09-12-01*/
     {3,"padya"},/*ajouté le 19-12-01*/
     {5,"raisebox"},
     {6,"framebox"},
     {6,"makebox"},
     {6,"footnotetext"},/*ajouté le 31-12-01*/
     {7,"cite"},
     {8,"chapter"},
     {8,"section"},
     {8,"subsection"},
   ...
   do {
	 ...
	 while (TRUE) {
	    ...
	    if (strstr(inbuf, "hindi\n") == inbuf+1) {
	       hindi_mode = TRUE;
	       lig_table[37].sym_new = 053;
	       lig_table[37].join_idx = 0;
	       lig_table[96].sym_new = 0200;
	       lig_table[96].join_idx = 0;
	    ...
	    if (strstr(inbuf, "sanskrit\n") == inbuf+1) {
	       hindi_mode = FALSE;
	       lig_table[37].sym_new = 0342;
	       lig_table[37].join_idx = 62;
	       lig_table[96].sym_new = 0233;
	       lig_table[96].join_idx = 0;
	       for (i = 0; i < N_NOLIGS; i++)
		 lig_table[nolig[i]].in_use = TRUE;
	       fprintf(f_out, "%%%s", inbuf);
	       linenumber++;
	       break;
	    }
	    ...
	    if (strstr(inbuf, "obeylines\n") == inbuf+1) {
	       printf("Warning: defunct @obeylines command used at line ");
	       printf("%d\n", linenumber);
	       fprintf(f_out, "%%%s", inbuf);
	       linenumber++;
	       break;
...
void put_word(void) {
   /*
    * The first hack here is to secure the correct representation
    * for "~nj~na". There really doesn't seem to be any better way to
    * achieve this. It can't be fixed by changing one or more of
    * the major data structures (char_table[], cons_table[],
    * lig_table[]), because that would affect other aspects of the
    * relevant characters' behaviour. It also can't be fixed by means
    * of a "general" extra algorithm inserted into (say) dnproc() or
    * put_ch(), because that would affect other groups such as "ttra"
    * (arguably desirable!), "ktra" (definitely undesirable).
    *
    * The second hack is to avoid hyphenation before any consonant
    * stopped with viraama.
    *
    * Essentially, both these problems arise because devnag processes
    * its data in a single stream, running top-to-bottom and
    * left-to-right, making it impossible to look ahead.
    *                                                   -- John Smith
    */
   char *w_ptr;
   char tmp[MEDBUF] = "^^V\\3E2w";
   do {                                                      /* hack 1 */
      w_ptr = strstr(word, "^^V>^^Z");
      if (w_ptr != NULL) strcpy(w_ptr, strcat(tmp, w_ptr+7));
   }
   while (w_ptr != NULL);
   if (do_hyphen) {                                          /* hack 2 */
      do {
...
/*
 * Append syl[] to word[], using the codes defined in out_string[]
 */
void put_syll(void) {
   /*
...
/*
 * Read and return the next character from inbuf[], taking appropraiate
