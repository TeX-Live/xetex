/*  $Header: /home/cvsroot/dvipdfmx/src/pdfobj.c,v 1.14 2004/01/30 18:34:22 hirata Exp $
...
int pdfobj_escape_str (char *buffer, int bufsize,
		       const unsigned char *s, int len, int remap)
{
  int result = 0, i;
  unsigned char ch;
  for (i=0; i<len; i++) {
...
static void write_string (FILE *file, const pdf_string *string)
{
  ...
  /*
   * If a PDF string contains much escaped chars, then we write it in hex.
   */
  if (nescc > (string->length)/3) {
    pdf_out_char(file, '<');
    for (i = 0; i < string->length; i++) {
      char hi, lo;
      hi = (s[i] >> 4) & 0x0f;
      lo = s[i] & 0x0f;
      pdf_out_char(file, ((hi >= 10) ? hi + 'W' : hi + '0'));
      pdf_out_char(file, ((lo >= 10) ? lo + 'W' : lo + '0'));
    }
    pdf_out_char(file, '>');
  } else {
    pdf_out_char(file, '(');
     ...
     */ 
    for (i = 0; i < string->length; i++) {
      int count = pdfobj_escape_str(format_buffer, FORMAT_BUF_SIZE, s+i, 1, 0);
      pdf_out(file, format_buffer, count);
    }
    pdf_out_char(file, ')');
  }
...
pdf_obj *pdf_new_name (const unsigned char *name)  /* name does *not* include the / */ 
{
  pdf_obj *result;
  ...
  result -> data = data;
  if (length != 0) {
    data -> name = NEW (length+1, unsigned char);
...
static void write_name (FILE *file, const pdf_name *name)
{
  unsigned char *s = name->name;
  int i, length;

  pdf_out_char(file, '/');
  length = (name->name == NULL) ? 0 : strlen(name->name);
  /*
...
void pdf_set_name (pdf_obj *object, unsigned char *name)
{
  pdf_name *data;
  unsigned length = strlen(name);
...
unsigned char *pdf_name_value (pdf_obj *object)
{
  pdf_name *data;
  if (object == NULL || object -> type != PDF_NAME) {
...
void pdf_add_stream (pdf_obj *stream, const char *stream_data, unsigned length)
{
  pdf_stream *data;
  if (stream == NULL || stream -> type != PDF_STREAM) {
