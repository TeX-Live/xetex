/*  $Header: /home/cvsroot/dvipdfmx/src/pdfdoc.c,v 1.14 2004/01/30 18:34:22 hirata Exp $
...
void pdf_doc_bop (const char *string, unsigned length)
{
  if (length > 0)
    pdf_add_stream (glob_page_bop, string, length);
}

void pdf_doc_this_bop (const char *string, unsigned length)
{
...
void pdf_doc_eop (const char *string, unsigned length)
{
  if (length > 0)
...
void pdf_doc_add_dest (const char *name, unsigned length, pdf_obj *array_ref)
{
#ifdef MEM_DEBUG
MEM_START
#endif
...
void pdf_doc_start_article (const char *name, pdf_obj *info)
{
...
void pdf_doc_add_bead (const char *article_name, pdf_obj *partial_dict)
{
...
void pdf_doc_add_to_page (const char *buffer, unsigned length)
{
  pdf_add_stream (this_page_contents, buffer, length);
}

void pdf_doc_init (const char *filename) 
{
#ifdef MEM_DEBUG
  MEM_START
#endif
...
void pdf_doc_creator (const char *s)
{
