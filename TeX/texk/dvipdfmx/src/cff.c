/*  $Header: /home/cvsroot/dvipdfmx/src/cff.c,v 1.10 2004/01/31 01:22:27 hirata Exp $
...
int cff_match_string (cff_font *cff, char *str, s_SID sid)
{
  card16 i;
...
card16 cff_glyph_lookup (cff_font *cff, char *glyph)
{
  ...
  case 1:
	...
	gid++;
	if (cff_match_string(cff, glyph, (s_SID)((charset->data).range1[i].first + n)))
	  return gid;
      }
    }
    break;
  case 2:
	...
	gid++;
	if (cff_match_string(cff, glyph, (s_SID)((charset->data).range2[i].first + n)))
	  return gid;
      }
    }
    break;
